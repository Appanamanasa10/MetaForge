...
import { Plus, Upload, Loader2, AlertCircle, Database, Layout } from 'lucide-react';
...
export default function RuntimeEntityPage() {
  ...
  const { data: recordsData, isLoading: isRecordsLoading, error } = useQuery<any>({
    ...
    queryFn: async () => {
      const url = `/api/records?entityId=${entityId}&page=${page}&limit=${limit}&search=${encodeURIComponent(debouncedSearch)}`;
  ...
  return (
    <DashboardLayout>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
          <div className="space-y-1">
            <div className="flex items-center space-x-2 text-xs text-slate-500">
              ...
              <span>/</span>
              <Link href={`/app/${appId}/builder`} className="hover:text-slate-300 transition-colors">{application?.name}</Link>
              <span>/</span>
            ...
            <div className="flex items-center space-x-3">
              <h1 className="text-2xl font-bold text-slate-100 flex items-center space-x-2">
                ...
                <span>{currentEntity?.name} Records</span>
              </h1>
              {!isRecordsLoading && recordsData?.pagination?.total !== undefined && (
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-400">
                  {recordsData.pagination.total} total
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="inline-flex items-center px-3.5 py-2 rounded-xl text-xs font-semibold bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-500/20 transition-all cursor-pointer"
            >
            ...
            <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/20 glass-panel">
              ...
              <div className="mt-5 pt-4 border-t border-slate-800">
                <Link
                  ...
                  className="w-full inline-flex justify-center items-center py-2 px-3 border border-dashed border-slate-800 rounded-xl text-xs text-slate-500 hover:text-slate-300 hover:border-slate-700 transition-colors"
                >
                  ✏️ Edit Schema
                </Link>
              </div>
            ...
            <RuntimeErrorBoundary>
              {error ? (
                ...
                <TableRenderer
                  ...
                  entityName={currentEntity?.name || 'Record'}
                  totalRecords={recordsData?.pagination?.total}
                />
              )}
            </RuntimeErrorBoundary>
          </div>
      ...
      <ConfirmDialog
        isOpen={!!deleteTarget}
        title="Delete Record"
        message="Are you sure you want to delete this record? This action is irreversible and cannot be undone."
        confirmLabel="Delete Record"
        onConfirm={() => deleteTarget && deleteRecordMutation.mutate(deleteTarget)}
